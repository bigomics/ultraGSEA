...
res2 <- ultragsea(fc, G, method='ztest')
res3 <- ultragsea(fc, G, method='cor')
...
Z <- cbind( ES=res1$ES, NES=res1$NES, ultragsea.z=res2$score,
  ultragsea.cor=res3$score, ks=stats)
...
library(npGSEA)
gset <- 
res3 <- npGSEA::npGSEA(x = X, y = y, set = gset)
